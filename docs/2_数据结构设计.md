### 1. Protobuf 定义 (`schemas/proto/scintirete/v1/scintirete.proto`)
...
message SearchResultItem {
  ...
  string id = 3; // 向量ID，用于 include_vector=false 时标识结果
...
scintirete-server -c ./configs/scintirete.toml --grpc.port 9091 --log.level debug
...
#   注意: 需要从 configs/scintirete.template.toml 复制配置文件模板
...
这是唯一的配置文件，结构清晰，分块管理，易于理解和修改。配置文件需要从模板 `configs/scintirete.template.toml` 复制创建。
...
#   删除一个或多个向量。
# scintirete-cli vector search <db_name> <coll_name> --vector '[0.3, 0.4, ...]' --top-k 5 [--efs 100] [--include-vector]
#   搜索相似向量。--efs 覆盖集合的默认 ef_search。--include-vector 控制是否返回向量数据（默认false以提高性能）。
...
# scintirete-cli text search <db_name> <coll_name> --query "A question to search" --top-k 5 [--model <model_name>] [--efs 100] [--include-vector]
#   自动嵌入查询文本并搜索。--include-vector 控制是否返回向量数据（默认false以提高性能）。
...
# 4. 如果需要向量数据，可以添加 --include-vector 参数
scintirete-cli -p "secret" text search my_app articles --query "What is Scintirete?" --top-k 3 --include-vector
```
