...
- 使用数字ID（如：1, 2, 3...）
- 使用 `auto` 让系统自动生成ID
- 向量ID现在是自增的 uint64 类型

**示例：**
```bash
vector insert vectors 1 "[1.0, 2.0, 3.0, 4.0]"
vector insert vectors auto "[1.0, 2.0, 3.0, 4.0]"  # 自动生成ID
vector search vectors "[1.1, 2.1, 3.1, 4.1]" 10
vector delete vectors 1 2
```
...
```bash
# 使用默认模型
text insert documents 1 "这是一段示例文本"
text insert documents auto "自动生成ID的文本"
text search documents "示例" 5

# 指定模型
text insert documents text-embedding-ada-002 2 "Another example" '{"category": "test"}'
