...
func (c *CLI) vectorCommand(args []string) error {
	...
	case "insert":
		if len(subArgs) < 2 {
			return fmt.Errorf("usage: vector insert <collection> <vector> [metadata]")
		}
...
func (c *CLI) insertCommand(args []string) error {
	if len(args) < 2 {
		return fmt.Errorf("usage: insert <collection> <vector> [metadata]")
	}
	...
	resp, err := c.client.InsertVectors(ctx, &pb.InsertVectorsRequest{
	...
	if len(resp.InsertedIds) > 0 {
		fmt.Printf("Vector inserted successfully with ID: %d\n", resp.InsertedIds[0])
	} else {
